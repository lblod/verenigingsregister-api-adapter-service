...
export class Adapter {
  ...
  async updateAddress(id, address, muHeaders) {
    ...
    this.compareValues(localAddressBeforeChange.attributes, primaryLocation.adres, ADDRESS_MAPPING);
    ...
    // update the address in the verenigingsregister

    const response = await this.beheersAPIClient.updateLocatie(vCode, locatieId, newAddress, eTag);
    ...
    // handle the response from the verenigingsregister
    if (response.status !== 200) {
      console.error('Error updating vereniging address:', response.status, response.statusText);
      throw new Error(`Failed to update vereniging address: HTTP ${response.status}`);
    }
    console.log('Vereniging address updated successfully:', response.data);
  }

  async updateContactPoint(id, contactPoint, muHeaders) {
    // Get the id of the association linked to the contact point
    const vCode = await this.findAssociationLocally(id, this.pathsToAssociation.associationContactPoint, muHeaders);
    const verenigingResponse = await this.beheersAPIClient.getVereniging(vCode);
    const vereniging = verenigingResponse.data;
    const eTag = verenigingResponse.headers['etag'];
    console.log('vereniging', JSON.stringify(vereniging, null, 2));
    // find the relevant id of the contactgegeven, and the e-tag
    // const contactgegevenId = vereniging.vereniging.contactgegevens.find((contactgegeven) =>
    //   contactgegeven['@id']?.endsWith(id) // @id has the format `cont:<uuid>` just checking endsWith in case the prefix changes.
    // )?.contactgegevenId;
    // UUID is not the same as the one in the URI.

    const localContactPointBeforeChange = (await this.jsonapi.getContactPoints(id, {}, muHeaders)).data;

    // find the relevant contact point in the vereniging.
    // Note that the uuid (i.e. json-api id) is not the same as the one in the URI(!). it is also different from the contactgegevenId, which is an integer.
    let contactgegeven = null;
    for (const cg of vereniging.vereniging.contactgegevens) {
      const uri = await fullUri(cg['@id'], vereniging);
      if (uri === localContactPointBeforeChange.attributes.uri) {
        contactgegeven = cg;
        console.log('Found contactgegeven:', cg);
        break;
        // } else {
        //   console.log('No match for contactgegeven:', uri, localContactPointBeforeChange.attributes.uri);
      }
    }

    if (!contactgegeven) {
      console.error('Contactgegeven not found for id', id);

      console.error('localContactPointBeforeChange', JSON.stringify(localContactPointBeforeChange, null, 2));
      throw new Error(`Contactgegeven with id ${id} not found in vereniging`);
    }

    console.log('contactgegeven', JSON.stringify(contactgegeven, null, 2));


    // Compare the contact point with the verenigingsregister response

    console.log('localContactPointBeforeChange', JSON.stringify(localContactPointBeforeChange, null, 2));
    if (!this.compareContactPoints(
      localContactPointBeforeChange.attributes,
      contactgegeven
    )) {
      console.error('Local contact point does not match the verenigingsregister response');
      throw new Error('Local contact point does not match the verenigingsregister response');
    }

    // Map the contact point to the verenigingsregister format
    const newContactPoint = this.createContactGegeven(contactPoint);
    console.log('newContactPoint', JSON.stringify(newContactPoint, null, 2));

    // Update the contact point in the verenigingsregister
    const response = await this.beheersAPIClient.updateContactgegeven(vCode, contactgegeven.contactgegevenId, newContactPoint, eTag);
    ...
    if (response.status !== 202) {
      console.error('Error updating contact point:', response.status, response.statusText);
      throw new Error(`Failed to update contact point: HTTP ${response.status}`);
    }
    console.log('Contact point updated successfully:', response.data);
    ...
    for (const [localKey, registerKey] of Object.entries(mapping)) {
      if (localValue[localKey] !== undefined && registerValue[registerKey] !== undefined) {
        if (localValue[localKey] !== registerValue[registerKey]) {
          console.log(
            `Mismatch for ${localKey}: local=${localValue[localKey]}, register=${registerValue[registerKey]}`
          );
          return false;
        } else {
  ...
  // Dedicated method because the mapping isn't just a simple key-value mapping.
  compareContactPoints(localContactPoint, registerContactPoint) {
    console.log('Make sure the local stored contact point does not conflict with the latest value from the register');

    if (registerContactPoint.contactgegeventype === 'E-mail') {
      return localContactPoint?.email === registerContactPoint.waarde
    } else if (registerContactPoint.contactgegeventype === 'Telefoon') {
      return localContactPoint?.telephone === registerContactPoint.waarde;
    } else if (registerContactPoint.contactgegeventype === 'Website') {
      return localContactPoint.website === registerContactPoint.waarde;
    } else if (registerContactPoint.contactgegeventype === 'SocialMedia') {
      console.log('SocialMedia contact point not handled yet');
      throw new Error('SocialMedia contact point not handled yet');
    } else {
      console.log('No matching contact point found');
      throw new Error('No matching contact point found');
    }
  }

  /**
   * Maps a contact point object to the format expected by the verenigingsregister.
   * Supports email, telephone, and website types.
   * Throws an error if no valid contact point is found.
   */
  createContactGegeven(contactpoint) {
    if (contactpoint.email) {
      return { type: 'E-mail', waarde: contactpoint.email };
    }
    if (contactpoint.telephone) {
      return { type: 'Telefoon', waarde: contactpoint.telephone };
    }
    if (contactpoint.website) {
      return { type: 'Website', waarde: contactpoint.website };
    }

    console.error('No valid contact point found');
    throw new Error('No valid contact point found');
  }
